...
import { LayoutDashboard, UploadCloud, FilePlus, Library, ChevronLeft, ChevronRight, LogOut, Calendar, Menu } from 'lucide-vue-next'
...
<template>
  <div class="flex h-screen bg-slate-900 text-slate-50 font-sans">
    ...
    <button 
      v-if="route.name !== 'Login' && !isMobileMenuOpen"
      @click="isMobileMenuOpen = true"
      class="md:hidden fixed top-3 left-3 z-40 p-2 bg-slate-800/80 backdrop-blur-sm rounded-lg border border-slate-700 text-slate-300 shadow-lg"
    >
      <Menu class="w-6 h-6" />
    </button>
    ...
    <div 
      v-if="route.name !== 'Login' && isMobileMenuOpen" 
      class="md:hidden fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-40" 
    ...
    <aside
      ...
      class="fixed inset-y-0 left-0 bg-slate-800 border-r border-slate-700 flex flex-col shrink-0 transition-transform duration-300 z-50 md:relative md:translate-x-0"
      :class="[
    ...
    >
      ...
      <nav class="flex-1 px-2 py-4 space-y-1">
        ...
        <RouterLink v-if="isAuthenticated" to="/import/bulk"
          class="flex items-center rounded-lg transition-colors px-3 py-3"
          :class="[
        ...
        <RouterLink v-if="isAuthenticated" to="/import/manual"
          class="flex items-center rounded-lg transition-colors px-3 py-3"
          :class="[
        ...
        <RouterLink v-if="isAuthenticated" to="/browser"
          class="flex items-center rounded-lg transition-colors px-3 py-3"
          :class="[
      ...
      <!-- Bottom Actions -->
      <div v-if="isAuthenticated" class="px-2 pb-4 shrink-0">
        <button @click="logout"
      ...
      <!-- Status Indicator -->
      <div v-if="isAuthenticated" class="p-4 border-t border-slate-700 shrink-0">
